...
@section('title', 'Reports - Staff Portal')
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200/50 dark:border-gray-700/50">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['applications']['total']) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">{{ $stats['applications']['this_month'] }} this month</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200/50 dark:border-gray-700/50">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">₦{{ number_format($stats['payments']['total_revenue'], 2) }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">{{ $stats['payments']['this_month'] }} payments this month</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200/50 dark:border-gray-700/50">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $stats['staff']['active'] }}</p>
                    <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">{{ $stats['staff']['with_roles'] }} with roles</p>
                </div>
        ...
        <form id="reportForm" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <select id="report_type" name="report_type" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white">
                    ...
                    @if(auth('transcript_staff')->user()->hasPermission('view_transcript_applications'))
                    <option value="applications">Applications Report</option>
                    @endif
                    @if(auth('transcript_staff')->user()->hasPermission('view_transcript_payments'))
                    <option value="payments">Payments Report</option>
                    @endif
                    @if(auth('transcript_staff')->user()->hasPermission('manage_transcript_staff'))
...
function generateReport(type, dateFrom = null, dateTo = null) {
    ...
    fetch(`{{ route('transcript.staff.reports.generate') }}?${params}`)
