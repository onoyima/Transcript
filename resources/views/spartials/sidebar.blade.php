...
<div id="staff-sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-blue-900 to-blue-800 transform -translate-x-full transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0">
    <div class="flex items-center justify-center h-20 shadow-md bg-blue-900">
        <div class="flex items-center">
            <div class="flex-shrink-0">
                <img class="h-10 w-10" src="{{ asset('images/logo/logo.png') }}" alt="Logo">
            </div>
            <div class="ml-3">
    ...
    <nav class="mt-5 flex-1 px-2 bg-transparent space-y-1">
        ...
        <a href="{{ route('transcript.staff.dashboard') }}"
           class="group flex items-center px-2 py-2 text-sm font-medium rounded-md {{ request()->routeIs('transcript.staff.dashboard') ? 'bg-blue-700 text-white' : 'text-blue-100 hover:bg-blue-700 hover:text-white' }} transition-colors duration-150">
            <i class="fas fa-tachometer-alt mr-3 flex-shrink-0 h-5 w-5"></i>
            Dashboard
        </a>

        <!-- Applications Management -->
        @can('view_transcript_applications')
        <div class="mt-6">
        ...
        <!-- System Settings -->
        @can('manage_transcript_settings')
        <div class="mt-6">
        ...
        <!-- Reports -->
        @can('generate_transcript_reports')
        <div class="mt-6">
            ...
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm font-medium text-gray-900">
                        {{ auth('transcript_staff')->user()->first_name }} {{ auth('transcript_staff')->user()->last_name }}
                    </p>
                    <p class="text-xs text-gray-500">
                        {{ auth('transcript_staff')->user()->roles->pluck('name')->join(', ') }}
                    </p>
                </div>
            </div>
        </div>
