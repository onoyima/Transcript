...
@section('title', 'Payment - Transcript Application')
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                    ...
                    <p class="text-lg font-semibold text-blue-600">#{{ $transcriptApplication->id }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="email" id="email" name="email" 
                               value="{{ $transcriptApplication->email }}"
                    ...
                    <button type="button" id="paystack-payment-btn" 
                        ...
                        <span id="payment-btn-text">Pay ₦{{ number_format($transcriptApplication->total_amount, 2) }}</span>
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="text-base font-bold text-green-600">₦{{ number_format($transcriptApplication->total_amount, 2) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-200">
                        ...
                        <div class="space-y-1">
                            <p class="text-xs text-gray-600">{{ $student->first_name }} {{ $student->middle_name }} {{ $student->surname }}</p>
                            <p class="text-xs text-gray-600">{{ $student->matric_number }}</p>
                            <p class="text-xs text-gray-600">{{ $transcriptApplication->email }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-lg p-4 mt-4">
                    ...
                    <a href="mailto:{{ config('mail.from.address') }}" class="text-xs text-blue-600 hover:text-blue-800">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    payButton.addEventListener('click', function() {
        ...
        fetch('{{ route("student.transcript.paystack.payment.initialize", $transcriptApplication->id) }}', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.status) {
                ...
                const handler = PaystackPop.setup({
                    ...
                    email: email,
                    amount: {{ $transcriptApplication->total_amount * 100 }}, // Amount in kobo
                    currency: 'NGN',
                    ref: data.data.reference,
                    callback: function(response) {
                        // Payment successful
                        window.location.href = '{{ route("student.transcript.paystack.callback") }}?reference=' + response.reference;
                    },
                    onClose: function() {
                        // Reset button state
